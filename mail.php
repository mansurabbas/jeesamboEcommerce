...
include("top.php");
$order_id=$_GET['id'];
$res=mysqli_query($conn,"SELECT distinct(order_details.id) ,order_details.*,products.product_name,products.image,products.price AS pp FROM order_details,products ,`order` WHERE order_details.order_id='$order_id' AND order_details.product_id=products.id");

$user_order=mysqli_fetch_assoc(mysqli_query($conn,"SELECT `order`.*, users.firstname,users.email  FROM `order`,users WHERE users.id=`order`.user_id AND `order`.id='$order_id'"));
